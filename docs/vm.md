...
**[導入](setup.md) を通したホストで、一般ユーザーのシェルから実行する** (`sudo -i` した root のシェルでは `kvm.sh` が止まる)。前提は `./kvm.sh up` 済みで `./kvm.sh virsh list` が通ること ([導入の手順 5](setup.md#5-動作確認する))。手順 1 は `sudo` のパスワードを聞かれる。`./kvm.sh` は内部で `sudo podman` を呼ぶので、タイムスタンプが切れていれば他のブロックでも聞かれる (答えてから続きを貼る)。手順 3 はホストのデスクトップにウィンドウが開く (ディスプレイの無いホストは飛ばす)。手順 0 で変数を設定したシェルで、上から順にコードブロックを貼る。理由・実測・落とし穴は[補足](#補足)にまとめてあり、実行するだけなら読まなくてよい。

| 手順 | 内容 |
|---|---|
| [0. 変数を設定する](#0-変数を設定する) | ISO のパス (必須) と VM 名・メモリ・vCPU・ディスク (任意) を決め、読み戻す |
| [1. ISO を置く](#1-iso-を置く) | `data/var-libvirt/images/` に `sudo cp` |
| [2. VM を作る](#2-vm-を作る) | `./kvm.sh virt-install --cdrom ...` |
| [3. 画面を開いてインストールする](#3-画面を開いてインストールする) | `./kvm.sh viewer` でインストーラを操作する |
...
ls -l "${ISO:?手順 0 の ISO が空のまま。値を入れて貼り直す}"
```
...
```bash
cd "${REPO:?手順 0 の REPO が空のまま。値を入れて貼り直す}" && sudo cp "${ISO:?手順 0 の ISO が空のまま。値を入れて貼り直す}" data/var-libvirt/images/
```
...
以降のブロックはこのディレクトリ (リポジトリ直下) で貼る。新しいシェルを開いたら手順 0 と、この `cd` をやり直す。
...
```bash
./kvm.sh virt-install --name "${VM_NAME:?手順 0 の VM_NAME が空のまま。値を入れて貼り直す}" --memory "${VM_MEMORY}" --vcpus "${VM_VCPUS}" --disk "size=${VM_DISK}" \
  --cdrom "/var/lib/libvirt/images/$(basename "${ISO:?手順 0 の ISO が空のまま。値を入れて貼り直す}")" --osinfo detect=on,require=off \
...
- `--graphics vnc` と `--noautoconsole` は固定 (理由は補足)。`--network` は省略でよい (省略時の挙動は補足。ブリッジにつなぐなら [bridge.md](bridge.md))
- `virt-install` はすぐ戻り、VM はインストーラが起動した状態 (`running`) になる

→ [補足](#手順-2-virt-install-のオプション)
...
画面を見るなら `./kvm.sh viewer "${VM_NAME}"` (手順 3 と同じ。ウィンドウが開く)。OS が起動してログイン画面になってから、ACPI で停止する (単独で貼る。起動途中の OS は ACPI に応じないことがある):
...
→ [補足](#手順-5-停止の仕組み-libvirt-guests)

---

## VM を削除する

**削除するディスクと、CD-ROM の中身を先に確認する** (手順 0 の変数を設定したシェルで、リポジトリ直下から):

```bash
./kvm.sh virsh domblklist "${VM_NAME:?手順 0 の VM_NAME が空のまま。値を入れて貼り直す}"   # vda = 消すディスク。sda に ISO が入ったままなら次のブロックで取り出す
```
...
sudo ls "${REPO:?手順 0 の REPO が空のまま。値を入れて貼り直す}/data/var-libvirt/images" "${REPO}/data/etc-libvirt/qemu"   # qcow2 と xml が消え、ISO は残っている
```
...
> | 変数 | 意味 | 例 |
> |---|---|---|
> | `${ISO}` | ダウンロードした ISO のホスト側パス。手順 1 で `data/var-libvirt/images/` にコピーし、手順 2 では `basename` だけを使う | `~/Downloads/AlmaLinux-10-latest-x86_64-dvd.iso` |
...
- **ISO は `data/var-libvirt/images/` に置く** — ホストの `data/var-libvirt` が `kvm` コンテナの `/var/lib/libvirt` なので、コンテナ内では `/var/lib/libvirt/images/` に見える。手順 2 の `--cdrom` に渡すのはコンテナ内のパス
...
- `ISO` はホスト側のパス。`ISO=~/Downloads/...` のように `~` で始めれば代入時に展開される (引用符で囲むと展開されない)。手順 1 でコピーし、手順 2 では `basename` だけをコンテナ内のパスに付ける
...
- 変数はそのシェルの中だけで有効。別の端末や SSH の張り直しの後は手順 0 を貼り直し、手順 1 の `cd` もやり直す
...
- ディスクファイルは `data/var-libvirt/images/<VM名>.qcow2`。`sudo ls -l` で所有者が root / qemu になっているのは仕様 (手順 1 の補足)

#### 手順 5: 停止の仕組み (libvirt-guests)
...
- **`down` 時に動いていた VM は次の `up` で起動しない**: `up` で起動させたい VM には `virsh autostart` を設定する (手順 5)
...
- boot ISO は先に [手順 1](#1-iso-を置く) で `data/var-libvirt/images/` に置く (手順 0 の `ISO` にその boot ISO のパスを入れる)
