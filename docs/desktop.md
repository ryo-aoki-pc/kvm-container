...
- **進め方**: 前提 (パスワード無しの `sudo podman`) を確かめ、`./kvm.sh install-desktop` で配置し、`./kvm.sh up` してからアクティビティで起動する。読者が書き換える値は無い (clone 先を変えたときだけ手順 1 の `REPO`)
- **状態**:
...
| sudoers | 一般ユーザーがパスワード無しで `sudo` を実行できる (読者が設定済み。[導入の実施前の状態](setup.md#実施前の状態))。手順 2 で確かめる |
...
- **NOPASSWD の意味**: `launch` のために podman を NOPASSWD にすると、そのユーザーはパスワード無しでホスト root 相当の操作ができる
  - 手順書はすべてこの設定を前提にしている ([導入の実施前の状態](setup.md#実施前の状態))。`launch` は端末が無いので `sudo -n` で実行し、通らなければデスクトップ通知になる ([SPEC.md 7 章](SPEC.md#7-セキュリティ考慮事項))
