...
| 手順 | 内容 |
|---|---|
| [0. 変数を設定する](#0-変数を設定する) | `NIC` (ブリッジに収容する物理 NIC) を入れ、読み戻す |
| [1. ブリッジを作る (NetworkManager)](#1-ブリッジを作る-networkmanager) | `nmcli` でブリッジ `br0` と bridge-slave を作り、NIC の接続をブリッジに切り替える |
| [2. ブリッジを登録して起動する](#2-ブリッジを登録して起動する) | `kvm` を止めてから `KVM_BRIDGE=… ./kvm.sh up`。libvirt ネットワーク `bridged` が登録される |
| [3. 動作確認する](#3-動作確認する) | `virsh net-list` で `bridged` が active |
| [4. VM をブリッジにつなぐ](#4-vm-をブリッジにつなぐ) | `virt-install` に `--network network=bridged` |
...
ip -br addr show "${NIC:?手順 0 の NIC が空のまま。値を入れて貼り直す}"
```
...
```bash
sudo nmcli connection add type bridge ifname "${BRIDGE:?手順 0 の BRIDGE が空のまま。手順 0 を貼り直す}" con-name "${BRIDGE}" ipv4.method auto
sudo nmcli connection add type bridge-slave ifname "${NIC:?手順 0 の NIC が空のまま。値を入れて貼り直す}" master "${BRIDGE}"
```

次のブロックは NIC の接続を落としてブリッジを上げる。**この NIC 越しに ssh でつないでいるとセッションが切れ、2 つ目のコマンドが走らないことがある**。コンソール (GNOME の端末) から、このブロックだけを単独で貼る。

```bash
sudo nmcli connection down "${NIC_CON:?手順 0 の NIC_CON が空のまま。手順 0 を貼り直す}" && sudo nmcli connection up "${BRIDGE}"
```
...
`ls -d` が `No such file or directory` なら、手順 2 の `up` は `!! KVM_BRIDGE=... is not a bridge on this host` で止まる。ブリッジの名前と状態を見直してから先へ進む。

→ [補足](#手順-1-nmcli-でブリッジを作る)
...
```bash
cd "${REPO:?手順 0 の REPO が空のまま。手順 0 を貼り直す}" && ./kvm.sh down kvm   # 動いている VM は ACPI で停止
```

次のブロックは `down` が終わってから貼る。sudo のタイムスタンプが切れていればパスワードを聞かれる。

```bash
KVM_BRIDGE="${BRIDGE:?手順 0 の BRIDGE が空のまま。手順 0 を貼り直す}" ./kvm.sh up
```
...
→ [補足](#手順-2-bridged-の登録と-kvm_bridge-の付け忘れ)
...
`net-list` に `bridged` が無ければ、`kvm` を止めずに `up` したか、`KVM_BRIDGE=` を付け忘れている。手順 2 をやり直す。

→ [補足](#手順-3-動作確認)

### 4. VM をブリッジにつなぐ

[vm.md 手順 2](vm.md#2-vm-を作る) の `virt-install` に `--network network=bridged` を足す。VM は LAN の DHCP から IP を取る (ホストと同じセグメント)。`--network` を省いたときの自動判定は[補足](#手順-4-vm-の接続)。既存の VM の付け替え (定義の `<source network='default'/>` を `bridged` にする) は本書では扱わない。`kvm` イメージにはエディタが無いので `virsh edit` はそのままでは使えず、`virsh dumpxml` → 編集 → `virsh define` の手順も未検証。
...
>
> `kvm.sh` 自身が読む環境変数 (`KVM_BRIDGE` など) は手順 0 の変数ではなく、`KVM_BRIDGE=br0 ./kvm.sh up` のようにコマンドの前に付ける。一覧は [setup.md の環境変数](setup.md#環境変数)。出力例・表の中の値は `<ブリッジ名>` / `<VM名>` / `<NIC>` のプレースホルダで書いてある。`<...>` を含むコマンドは bash のコードブロックに置いていない。
>
> パスワード・鍵・トークンは扱わない。
...
- `NIC_CON` は手順 0 の時点の値を保持する。手順 1 で NIC の接続を切り替えたあとにシェルを開き直して手順 0 を貼ると、NIC に付いている接続は `bridge-slave-<NIC>` なので `NIC_CON` はその名前になる。ロールバックで元の接続名が要るときは、手順 0 の読み戻しの出力を控えておく
...
- `down kvm` は動いている VM を `libvirt-guests` が ACPI でシャットダウンしてから止める (120 秒で電源断、`podman rm -t` は 180 秒。[SPEC.md 5.2](SPEC.md#52-停止シーケンス-kvmsh-down))。`down` の時点で動いていた VM は次の `up` で起動しない (`virsh autostart` の VM を除く。[vm.md](vm.md#5-起動と停止と自動起動))。`down kvm` の間 `kvm-gui` は残るが libvirt に届かない状態になり、次の `up` で `kvm` が起動して共有の `/run/libvirt` が空にされると復帰する。`down` (両方) でもよい
...
- ブリッジがホストに無い (手順 1 の前、再起動でブリッジが上がっていない、名前の打ち間違い) と、`up` は `!! KVM_BRIDGE=<ブリッジ名> is not a bridge on this host. Create it first (see docs/bridge.md)` と本書を指して exit 1 する。ホスト側のブリッジを直してから `up` し直す
