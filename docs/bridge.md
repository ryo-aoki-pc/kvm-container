...
> - **GNOME にログインした端末 (コンソール) から行う**。手順 3 は NIC の接続をブリッジに切り替えるので、その NIC 越しの ssh は切れる
...
- 手順の後: VM を作るのは手順 8 ([vm.md](vm.md) を `VM_NETWORK=bridged` で通す)。戻すときは[ロールバック](#ロールバック)

> [!WARNING]
> **本書は通しで実行していない** ([対象と検証環境](#対象と検証環境))。
>
> - 記述は `kvm.sh` の実装 (`check_host_network` / `sync_bridged_network`) から書いた
> - 手順 2・3 の nmcli は物理ホストで本実行していない (検証に使った物理ホストは NIC が無線のみ)
...
1. 変数を設定する (`NIC` は必ず値を入れる)。
   ...
   - `BRIDGE` と `REPO` は、既定のままでよければそのまま貼る。`NIC_CON` は `NIC` から自動で入る
   - 最後に値を読み戻し、リポジトリ直下に移る。`NIC` が空のままだと最後の行で止まる
   ...
   - 新しいシェルを開いたら (SSH を張り直したあとも)、手順 1 の 2 つのブロックを貼り直してから先へ進む。**手順 3 の後に貼り直すと `NIC_CON` の値が変わる** (この手順の補足)
   ...
   - `NIC_CON` は手順 1 の時点の値を保持する。手順 3 で NIC の接続を切り替えたあとにシェルを開き直して手順 1 を貼ると、NIC に付いている接続は `bridge-slave-<NIC>` なので `NIC_CON` はその名前になる。ロールバックで元の接続名が要るので、手順 1 の読み戻しの出力を控えておく
...
1. ブリッジが上がり、`kvm.sh` がブリッジと判定できるか確かめる。
   ...
   - `ls -d` が `No such file or directory` なら、手順 6 の `up` は `!! KVM_BRIDGE=... is not a bridge on this host` で止まる
...
1. ブリッジを登録するため、先に `kvm` を止める。
   ...
   - `kvm` が動いたままだと、手順 6 の `up` は `>> kvm is already running` で戻り、`bridged` は登録されない (手順 6 の補足)
   - **次の手順は、`down` が終わってから貼る**

   <details>
   <summary>補足: <code>down kvm</code></summary>

   - `down kvm` は動いている VM を `libvirt-guests` が ACPI でシャットダウンしてから止める (120 秒で電源断、`podman rm -t` は 180 秒。[SPEC.md 5.2](SPEC.md#52-停止シーケンス-kvmsh-down))。`down` の時点で動いていた VM は次の `up` で起動しない (`virsh autostart` の VM を除く。[vm.md 手順 9](vm.md#実施手順))。`down kvm` の間 `kvm-gui` は残るが libvirt に届かない状態になり、次の `up` で `kvm` が起動して共有の `/run/libvirt` が空にされると復帰する。`down` (両方) でもよい
...
1. `KVM_BRIDGE` を付けて `kvm` を起動し、ブリッジを `bridged` として登録する。
   ...
   - `bridged` の登録は `sync_bridged_network` ([SPEC.md 5.6](SPEC.md#56-ブリッジ同期-sync_bridged_network)) が行う。これは `start_kvm` が `podman run` で `kvm` を新しく起動し、libvirt の readiness を確認した直後にしか走らない。`kvm` がすでに動いていると `start_kvm` は `>> kvm is already running` で先に戻るので (`kvm.sh` の `start_kvm` 冒頭)、`KVM_BRIDGE=` を付けて `up` しても何も起きない。そのため手順 5 の `down kvm` と手順 6 の `KVM_BRIDGE=… up` に分けてある
   - 起動前に `check_host_network` が `/sys/class/net/<ブリッジ名>/bridge` の有無を確かめ、無ければ `!! KVM_BRIDGE=<ブリッジ名> is not a bridge on this host. Create it first (see docs/bridge.md)` で exit 1 する ([SPEC.md 2.4](SPEC.md#24-起動前に確認されるホスト資源-check_host_networkkvm-の起動時))。手順 2・3 を飛ばしたときに出る
...
1. `bridged` が登録されたか確かめる。
   ...
   - `net-list` に `bridged` が無ければ、`kvm` を止めずに `up` したか、`KVM_BRIDGE=` を付け忘れている。手順 5・6 をやり直す
...
1. [vm.md](vm.md#実施手順) を手順 1 から `VM_NETWORK=bridged` で通して VM を作る。

   - vm.md 手順 1 の任意ブロックでは `VM_NETWORK=bridged` にして貼る
   - vm.md 手順 4 の `virt-install` が `--network network=bridged` で VM を作る
   - VM は LAN の DHCP から IP を取る (ホストと同じセグメント)

1. vm.md 手順 4 の後に、VM が `bridged` につながっていることを確かめる。

   ```bash
   ./kvm.sh virsh domiflist "${VM_NAME:?vm.md 手順 1 の VM_NAME を設定してから貼る}"   # Type が network、Source が bridged
   ```

   - vm.md 手順 1 の変数を設定したシェルで貼る
   ...
   - `--network` を省くと、virt-install はホストの既定経路がブリッジ (`bridge0` など) 上にあればそのブリッジに、無ければ `default` (NAT、192.168.122.0/24) につなぐ (`kvm` はホストのネットワーク名前空間を共有するので、ホストのブリッジが見える)。手順 3 のあとはホストの既定経路がブリッジ上にあるので、`--network` を省いた VM もそのブリッジに直接つながり得る (この経路は `bridged` を経由しない)。vm.md 手順 4 は `VM_NETWORK` で `--network` を明示する ([SPEC.md 8 章](SPEC.md#8-既知の制限事項))
...
> [!WARNING]
> **以下は本実行していない。この節の手順 4 でブリッジを消すと、その NIC 越しの ssh は切れる。** コンソール (GNOME の端末) から行う。
>
...
1. `KVM_BRIDGE` を付けずに `kvm` を起動し直し、`bridged` を削除する。
...
1. `bridged` が消えたか確かめる。
   ...
   - `NIC_CON` は手順 1 で控えた元の接続名。新しいシェルなら、手順 1 を貼り直さずに `NIC_CON=` で控えた名前を入れてから、この節の手順 4 を貼る (手順 1 を貼り直すと `bridge-slave-…` の名前になる)
...
1. `NIC_CON` に元の接続名が入ったシェルで、ホストのブリッジと bridge-slave を消し、NIC の元の接続を上げる。
...
- **状態**:
  ...
  - **手順 2・3 の nmcli 手順も物理ホストで本実行していない** (物理 AlmaLinux 10.2 + GNOME での通しの確認 PR #15 `ae650c0` は NIC が無線のみで `KVM_BRIDGE` を試していない)
  - README の例を変数形に書き換えたもので、その形では再実行していない行: 手順 1 の `NIC_CON` の式 (`awk -F: -v d="${NIC}" '$2==d{print $1}'`)、手順 2・3 の nmcli 行、手順 6 の `KVM_BRIDGE="${BRIDGE}" ./kvm.sh up`
  - 新規の確認行で本実行していないもの: 手順 1 の読み戻しと `cd`、手順 4 の `ip -br addr show` / `ls -d`、手順 5 の `./kvm.sh down kvm`、手順 7 の `net-dumpxml`、手順 9 の `domiflist`、ロールバックの各手順 (手順 7 の `net-list` は README にあった行)
...
| 手順 2・3 の nmcli 手順 | 未検証。物理 AlmaLinux 10.2 + GNOME (PR #15 `ae650c0`) は NIC が無線のみ |
...
> - 手順 8・9 の `VM_NAME` と `VM_NETWORK` は [vm.md](vm.md) の手順 1 の変数
...
- [setup.md](setup.md) の[環境変数](setup.md#環境変数)と[注意点](setup.md#注意点)、[vm.md](vm.md) の[手順 1・4](vm.md#実施手順) (`VM_NETWORK`)
...
- 手順 2・3 の nmcli 手順 (物理ホスト、有線 NIC) の本実行。ブリッジが DHCP で NIC と同じ IP を引き継ぐか、ブリッジを上げたあとの ssh の復帰
- 手順 1 の `NIC_CON` の式 (`awk -v d="${NIC}"` 形) と、手順 1〜6 の変数形・新規の行
- 手順 7 の `net-dumpxml bridged` の実際の出力
- 物理ホストのブリッジに `--network network=bridged` (vm.md の `VM_NETWORK=bridged`) でつないだ VM が LAN の DHCP から IP を取ること、手順 9 の `domiflist` の出力
