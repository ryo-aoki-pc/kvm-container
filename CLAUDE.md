...
(sudoers の設定は読者の責任で、書き方は載せない。前提は `docs/setup.md` の「実施前の状態」に書き、他の 3 本はそこを参照する)。ブロックを分けるのは
sudo 以外の理由 (ビルド・`up` / `down` / `cp` の完了待ち、`[y/N]` の応答待ち、ssh の切断、ウィンドウが開く) があるときだけにする。
