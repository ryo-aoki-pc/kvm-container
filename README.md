...
- 上から順にコードブロックを貼れば通るようにする。リポジトリの clone も[導入](docs/setup.md)の手順に含め、画面の有無などの分岐は読者に選ばせずブロックの中で判定する。sudo のパスワードを聞くコマンドは単独のブロックにする
- 手順の本文は操作と確認点の箇条書きにとどめ、長い説明は折り畳みか後半の「補足」へ回す
- 実行の前提 (実行するユーザーと端末、前提の手順書、対話入力のある手順) は `## 実施手順` の冒頭に `> [!IMPORTANT]` で示す。通しで実行していない手順書は、検証範囲を `> [!WARNING]` で示す
